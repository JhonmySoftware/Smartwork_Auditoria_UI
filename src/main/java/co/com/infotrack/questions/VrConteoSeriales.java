...
public class VrConteoSeriales implements Interaction {

    public static VrConteoSeriales one() {
        return Instrumented.instanceOf(VrConteoSeriales.class).withProperties();
    }
    ...
    public <T extends Actor> void performAs(T actor) {
        ...
        while (!ListaCantidad01.isEmpty()) {
            for (Iterator<Target> iterator = ListaCantidad01.iterator(); iterator.hasNext(); ) {
                ...
                if (contador3 % 10 == 0 && ObGestionarAuditoria.SiguientePagina.resolveFor(actor).isClickable()) {
                    try {
                        actor.attemptsTo(
                                Wait.until(
